...
server.get("/upbinqing",(req,res)=>{
    ...
    var obj=req.query;
    var sql="UPDATE ys_binqin SET ? WHERE uname is null"
    pool.query(sql,[obj],(err,result)=>{
        console.log(result);
        if(err) throw err;
        if(result.affectedRows>0){
...
server.get("/upbinqing2",(req,res)=>{
    ...
    var obj=req.query;
    var sql="UPDATE ys_binqin SET ? WHERE ddid is null"
    pool.query(sql,[obj],(err,result)=>{
        console.log(result);
        if(err) throw err;
        if(result.affectedRows>0){
...
//查询订单详情
server.get("/sebinqing",(req,res)=>{
    var ddid=req.query.ddid;
    var sql="SELECT * FROM ys_binqin WHERE ddid=?"
    pool.query(sql,[ddid],(err,result)=>{
        if(err) throw err;
        if(result.length>0){
...
//查询用户订单详情
server.get("/sebinqing2",(req,res)=>{
    var uid=req.session.uid;
    if(!uid){
        res.send({code:-1,msg:"请登录"});
        return;
    }
    var sql="SELECT * FROM ys_binqin WHERE uid=?"
    pool.query(sql,[uid],(err,result)=>{
        if(err) throw err;
        if(result.length>0){
            res.send({code:1,msg:"查询成功",data:result})
...
server.get("/delete",(req,res)=>{
    ...
    var uname=req.query.uname;
    var sql="DELETE FROM ys_family WHERE uid=? AND uname=?"
